...
export function renderPopup(onRender: () => void, closePopup: () => void, submitComment: () => void): void {
  ...
  if (isNew) {
    ...
    popup.addEventListener('click', (e) => {
      ...
      else if (t.dataset.action === 'submit-popup') { submitComment(); }
    });
  ...
  h += '<textarea placeholder="コメントを入力" data-action="popup-textarea">' + esc(state.popupContent) + '</textarea>';
  h += '<div class="fb-popup-actions">';
  h += '<button class="cancel" data-action="cancel-popup">キャンセル</button>';
  h += '<button class="submit" data-action="submit-popup">送信</button>';
  h += '</div>';
  popup!.innerHTML = h;

  const rect = state.selectedRect;
  const pw = 400, ph = 260, m = 12;
